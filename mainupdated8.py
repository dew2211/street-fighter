...
def main():
    ...
    kalasize = 150
    kalaoffset = [67, 53]
    blackdata =  [kalasize, kalascale, kalaoffset]
    ...
    blacksprite = pygame.image.load("hello/image/bossmans/kala.png").convert_alpha()
    tribalsprite = pygame.image.load("hello/image/bossmans/somalian.png").convert_alpha()
    ...
    fightimg = pygame.image.load("hello/image/buttons/106-1069038_street-fighter-fight-logo-hd-png-download-removebg-preview.png").convert_alpha()
    ...
    def fighter_shet():
        if f1 == True:
            Fighter(1, 200 , 310, False,  warriordata, warriorsprite, warriorsteps, sword_fx )
        if f2 == True:

            Fighter(2, 700 , 310, True, wizarddata, wizardsprite, wizardsteps, magic_fx)
        if f3 == True:

            Fighter(3, 200 , 310, False,  odendata, odensprite, odensteps, sword_fx )
    ...
    kala_button = Button(SCREEN_WIDTH // 2 + 150, SCREEN_HEIGHT // 2 + 25, f4img_transform)
    ...
    while Run:
      ...
      if main_menu == True:
        ...
        if score[0] or score[1] == 1:
            draw_end_bg()
            screen.blit(gameover_transform, (250, 0))
